...
".sdrbar INPUT.fok{border-color:var(--good)}"
".sdrbar INPUT.fbad{border-color:var(--bad)}"
".sdrbar INPUT.fadj{border-color:var(--warn)}"
".rngmsg{font-weight:600}"
".rngmsg.ok{color:var(--good)}.rngmsg.adj{color:var(--warn)}.rngmsg.bad{color:var(--bad)}"
...
"<DIV ID='spectrum' CLASS='pane on'><DIV CLASS='wrap specwrap'>"
  "<DIV CLASS='card'>"
    "<DIV CLASS='sdrbar'>"
      ...
      "<INPUT CLASS='sp' TYPE='NUMBER' STEP='any' ID='loinp' ONINPUT=\"markDirty('lo')\" ONKEYDOWN='rngKey(event)'>"
      "<SELECT CLASS='sp' ID='lou' ONCHANGE=\"convUnit('lo')\"><OPTION>kHz</OPTION><OPTION>MHz</OPTION></SELECT>"
      "<SPAN CLASS='sp'>End</SPAN>"
      "<INPUT CLASS='sp' TYPE='NUMBER' STEP='any' ID='hiinp' ONINPUT=\"markDirty('hi')\" ONKEYDOWN='rngKey(event)'>"
      "<SELECT CLASS='sp' ID='hiu' ONCHANGE=\"convUnit('hi')\"><OPTION>kHz</OPTION><OPTION>MHz</OPTION></SELECT>"
      "<BUTTON CLASS='sp' ONCLICK='applyRange()'>Set range</BUTTON>"
      "<SPAN CLASS='rngmsg sp' ID='rngmsg'></SPAN>"
      "<SPAN CLASS='sp' ID='bandhint'></SPAN>"
...
/* per-field dirty state: while dirty the refresh loop won't overwrite the typed
   value (even after blur), until the user applies or clears it. */
...
"function rngMsg(kind,txt){var m=$('rngmsg');if(m){m.className='rngmsg sp'+(kind?' '+kind:'');m.textContent=txt||'';}}"
"function fldFx(l,h,c){var li=$('loinp'),hi=$('hiinp');if(li)li.className='sp'+(l?' '+l:'');if(hi)hi.className='sp'+(h?' '+h:'');}"
"function applyRange(){var lo=parseFloat($('loinp').value),hi=parseFloat($('hiinp').value);"
  ...
  "if(gBandLoHz&&gBandHiHz){if(clh<gBandLoHz){clh=gBandLoHz;reason='Init raised to band min';fl='fadj';}"
  ...
  "wfRows=[];fetch('/api/scan?lo='+clh+'&hi='+chh).catch(function(){});if(!scanning)scanOnce();"
  "dirtyLo=dirtyHi=false;"
  "if(reason){$('loinp').value=fmtIn(clh,$('lou').value);$('hiinp').value=fmtIn(chh,$('hiu').value);"
    "fldFx(fl,fh);rngMsg('adj',reason+' \\u2192 using '+axFmt(clh)+' \\u2013 '+axFmt(chh));}"
...
"function winPreset(wkhz){var c=gFreqHz||(lastScan?(lastScan.startHz+lastScan.stepHz*(lastScan.count-1)/2):0);if(!c)return;"
  ...
  "applyRange();}"
"function updateBandHint(){var h=$('bandhint');if(!h)return;if(!gBandLoHz||!gBandHiHz){h.textContent='';return;}"
